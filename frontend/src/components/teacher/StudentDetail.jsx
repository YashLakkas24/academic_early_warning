...
export default function StudentDetail({ status, student, riskLevel, onBack }) {
  ...
  const interventionReasons = Array.isArray(intervention.reasons)
  ...
  const aiAnalysis =
    student.analysis || "AI analysis is currently unavailable.";

  const performanceTrend = student.performance_trend || "STABLE";

  const trendIcon = TREND_ICON[performanceTrend] || TREND_ICON.STABLE;

  const formattedTrend =
    performanceTrend.charAt(0) + performanceTrend.slice(1).toLowerCase();
  ...
  return (
    ...
    ===================================================== */}
      ...
      <div className="detail-section">
        ...
        <div className="detail-section-body">{aiAnalysis}</div>
      </div>
      {/* =====================================================
          AI INTERVENTION
          ===================================================== */}
      ...
      <div className="detail-section">
        <div className="detail-section-label">AI Intervention</div>

        <div className="detail-section-body">
          {interventionReasons.length > 0 ? (
            <ul>
              {interventionReasons.map((reason, index) => (
                <li key={index}>{reason}</li>
              ))}
            </ul>
          ) : (
            <p>AI intervention information is currently unavailable.</p>
          )}
        </div>
      </div>
      ...
      <div className="detail-section">
        ...
        <div className="detail-section-body suggestion">{aiSuggestion}</div>
      </div>
    </div>
  );
}
